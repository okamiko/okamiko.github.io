...
function calculateGrade() {
    ...
    // Step 1: Take table inputs and convert them to a number(float)
    ...
    // Step 2: After grabbing the values, calculate the finalAverage by using Formula
    ...
    // Step 0: Make sure table inputs are in the range 0-100 and it is a valid number(not some letters)
    if (isNaN(finalAverage) || hwAvg < 0 || hwAvg > 100 || midExam < 0 || midExam > 100 || finalExam < 0 || finalExam > 100 || ACR < 0 || ACR > 100) {
        ...
        displayGrade.innerHTML = "Score is: " + finalAverage + " Student earned an 'F'. Student must retake the course";
    }
}
